...
window.descargarCertificadosMasivo = async function () {
  ...
  const mesVal  = document.getElementById('cert-bulk-mes').value; // "YYYY-MM" o vacío
  const status  = document.getElementById('cert-bulk-status');

  if (!cursoId) { alert('❌ Selecciona un curso'); return; }

  status.textContent = '🔄 Consultando aprobados...';
  ...
  let q = supabase
    ...
    .select(`
      puntaje, created_at,
      profiles!envios_formulario_usuario_id_fkey(
        nombres, apellidos, documento_numero, documento_tipo,
        cargos(nombre), empresas(nombre)
      ),
      formularios(tipo)
    `)
    .eq('id_curso', cursoId)
  ...
  // Filtrar solo exámenes y quedarse con la última nota por usuario
  const mapaUltimo = {};
  for (const e of (envios || [])) {
    if (e.formularios?.tipo !== 'examen') continue;
    const uid = e.profiles?.documento_numero || e.profiles?.nombres;
    if (!mapaUltimo[uid] || new Date(e.created_at) > new Date(mapaUltimo[uid].created_at)) {
      mapaUltimo[uid] = e;
    }
  }
  const aprobados = Object.values(mapaUltimo);

  if (!aprobados.length) {
    status.textContent = '⚠️ No se encontraron trabajadores aprobados con esos filtros.';
    return;
  }
  ...
  const { data: curso } = await supabase
    .from('cursos')
    .select('titulo, duracion, codigo_prefijo')
    .eq('id', cursoId)
    .single();

  // Obtener certificados existentes para tener los códigos reales
  const { data: certs } = await supabase
    .from('certificados')
    .select('usuario_id, codigo, nota')
    .eq('curso_id', cursoId);

  const mapaCodigos = {};
  for (const c of (certs || [])) mapaCodigos[c.usuario_id] = c;

  const zip    = new window.JSZip();
  const folder = zip.folder('Certificados');
  const duracion = curso?.duracion ? `${curso.duracion} hora${curso.duracion > 1 ? 's' : ''}` : '';

  status.textContent = `📄 Generando 0 / ${aprobados.length} PDFs...`;

  for (let i = 0; i < aprobados.length; i++) {
    const e       = aprobados[i];
    const perfil  = e.profiles || {};
    const certInfo = mapaCodigos[perfil.id] || {};
    ...
    const notaTexto      = certInfo.nota
      ? String(certInfo.nota)
      : (e.puntaje?.toFixed ? e.puntaje.toFixed(1) : String(e.puntaje || ''));
    ...
    const nombreArchivo = `${dni}_${nombreCompleto.replace(/\s+/g, '_')}.pdf`;
    folder.file(nombreArchivo, pdfBlob);

    status.textContent = `📄 Generando ${i + 1} / ${aprobados.length} PDFs...`;
  }
  ...
  status.textContent = `✅ Descargado ZIP con ${aprobados.length} certificados.`;
};
